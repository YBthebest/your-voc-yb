...
if(isset($_GET['id'])){
	...
	$id_combiner = $_POST['id'];
}

$liste = getListeById($id_combiner);
foreach($liste as $res_liste){
	$titre = $res_liste->titre();
	$mots = $res_liste->listeMot();
	$auteur = $res_liste->membre();
	$commentaire = $res_liste->commentaire();
	$categorie = $res_liste->categorie();
	$categorie2 = $res_liste->categorie2();
	$date = $res_liste->date();
	$id = $res_liste->id();
}
?>
...
<div id="content">
	<div id="bloc">
		<div id="text-center">
			...
			<p>
				<h3>Combiner <a href="afficher?id=<?php echo $id ?>"><?php echo $titre ?></a> avec une autre liste.</h3><br />
				<?php
				if(isset($_POST['requete']) && $_POST['requete'] != NULL OR isset($_GET['requete'])) {
					...
					if(!empty($resultats)) // si le nombre de résultats est supérieur à 0, on continue
					{
						...
						for($i=1; $i<=$nombreDePages; $i++) //On fait notre boucle
						{
							...
							{
								echo ' <a href="new_combiner?nb_page='.$i.'&id='.$id.'&requete='.$search.'&categorie='.$categorie.'&sur='.$critere.'&critere='.$tri.'">'.$i.'</a> ';
							}
						}
						echo '</p>';
						$i = 1;
						?>
						<form method="post" action="new_combiner?nb_page=<?php echo $i ?>&id=<?php echo $id ?>&requete=<?php echo $search ?>&categorie=<?php echo $categorie ?>&sur=<?php echo $critere ?>&critere=<?php echo $tri ?>" >
						...
						foreach($resultats as $donnees) {
							echo "".$i.".";
							?>
							<a href="afficher?id=<?php echo $donnees->id(); ?>"><?php echo $donnees->titre(); ?></a> <small>entré le <?php echo $donnees->date() ?> <form method="get"><input type="hidden" name="id" value="<?php echo $id ?>"/><input type="hidden" name="id_2" value="<?php echo $donnees->id() ?>"/><input type="submit" value="Combiner avec cette liste"/></form><br/>
						...
						?><br/>
						<br/>
						<a href="new_combiner?id=<?php echo $id ?>">Faire une nouvelle recherche</a></p>
						<?php
					...
					{ // de nouveau, un peu de HTML
						...
						<p>Nous n'avons trouvé aucun résultat pour votre requête "<?php  echo $search ?>". <a href="new_combiner?id=<?php echo $id ?>">Réessayez</a> avec autre chose.</p>
						<?php
					...
					<p>Vous allez faire une recherche dans notre base de données concernant les listes publiques.</p>
					 <form action="new_combiner" method="Post">
